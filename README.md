...
| 기준 배경 (`background.png`) | 전경 추출 결과 (`output_face.png`) |
|:---:|:---:|
| ![Background](background.png) | ![Foreground](output_face.png) |

### 2. 녹화 동영상 비교 (Video Comparison)

| 원본 녹화 (`output.gif`) | 배경 제거 녹화 (`output_no_background.gif`) |
|:---:|:---:|
| ![Original Video](output.gif) | ![Foreground Video](output_no_background.gif) |
...
- **실시간 민감도 제어:** 차영상(Background Subtraction)의 민감도를 조절하여 이마 또는 그림자 소실 방지
- **원본 영상 녹화:** `output.avi` (밝기/대비 조절 반영)
- **배경 제거 영상 녹화:** `output_no_background.avi`
- **전경 스냅샷 저장:** 현재 화면에서 추출된 객체를 `output_face.png`로 저장
...
| `F` 또는 `2` | 배경 제거된 객체 이미지 저장 (`output_face.png`) |
| `ESC` | 프로그램 종료 |
...
1. **가우시안 블러(Gaussian Blur)**  통한 사전 노이즈 제거
